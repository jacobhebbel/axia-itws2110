...
def start(data: pd.DataFrame) -> dict:
    ...
    # Vectorized prices padded with nans
    start_prices = close_prices.apply(lambda x: x[x.first_valid_index()] if x.first_valid_index() is not None else np.nan)
    end_prices   = close_prices.apply(lambda x: x[x.last_valid_index()]  if x.last_valid_index()  is not None else np.nan)
